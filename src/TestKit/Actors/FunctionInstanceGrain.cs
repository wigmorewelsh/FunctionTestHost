...
{
    Started, LoadingMetadata, LoadingFunctions, Ready, Running, Sleeping, Stopped
}

public enum TriggerEnum
{
    ObserverAdded, LoadedMetadata, LoadedFunctions, 
    InvocationRequest, InvocationResponse, Stop,
    InvocationStarted,
    EmptyQueue
}
...
public class FunctionInvocationRequest
{
    ...
    public FunctionInvocationRequest(Guid taskId, StreamingMessage streamingMessage)
    {
        ...
        _taskCompletionSource = new TaskCompletionSource<InvocationResponse>(TaskCreationOptions.RunContinuationsAsynchronously);
    }
...
internal class FunctionInstanceGrain : Grain, IFunctionInstanceGrain
{
    ...
    public FunctionInstanceGrain(ILogger<FunctionInstanceGrain> logger, IEnumerable<IDataMapperFactory> dataMapperFactories)
    {
        _dataMapperFactories = dataMapperFactories;
        
        _stateMachine.OnTransitioned((transition) =>
        {
            logger.LogInformation($"State: {transition.Source} -> {transition.Destination} via {transition.Trigger}");
        });
    ...
    public async Task InitMetadata(AzureFunctionsRpcMessages.FunctionMetadataResponse message)
    {
        foreach (var loadRequest in message.FunctionMetadataResults)
        {
            ...
            var directEndpointGrain = GrainFactory.GetGrain<IFunctionEndpointGrain>(functionName + "/" + loadRequest.Name);
    ...
    {
        if(message.FunctionMetadataResponse is { } functionMetadataResponse && functionMetadataResponse.Result.Status is StatusResult.Types.Status.Success)
